...
def find_frequent_itemsets(transactions, minimum_support, include_support=False):
    ...
    just the itemsets.  ## include_support (要不要顯示每個count到次數)
    '''
    items = defaultdict(lambda: 0) # mapping from items to their supports  ## 將相同 key 的 vlaue(int) 變成一個list
    ...
    def clean_transaction(transaction):
        transaction = filter(lambda v: v in items, transaction)  ## 把transaction中沒有超過 mimsup(由items中的item判斷, 已經過要大於mimsup) 的item拿掉
        transaction.sort(key=lambda v: items[v], reverse=True)  ## sort by value
        return transaction
